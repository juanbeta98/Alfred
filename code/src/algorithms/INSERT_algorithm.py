...
def insert_single_labor(
    ...
    early_buffer: float,
    workday_end_dt,
    service_block_after_first_afterhours_nontransport: bool,
    instance: str,
    selection_mode: str = "min_total_distance",
    update_dist_dict: bool = True,
):
    """
    Insert a single labor (transport or non-transport), returning:
    - success_flag
    - updated labors_df
    - updated moves_df
    - new_end_time
    ...
    is_transport = labor["labor_category"] == "VEHICLE_TRANSPORTATION"
    ...
    if is_transport:
        ...
        if curr_end_time and curr_end_time > workday_end_dt:
            return True, True, labors_df, moves_df, curr_end_time, curr_end_pos

        success, new_labors, new_moves, new_time, new_pos, _ = commit_labor_insertion(
            labors_algo_df=labors_df,
            moves_algo_df=moves_df,
            new_labor=labor,
            ...
            update_dist_dict=update_dist_dict,
        )

        return success, False, new_labors, new_moves, new_time, new_pos

    # ---------------------------------------------------
    # NON-TRANSPORT LABOR
    # ---------------------------------------------------
    if curr_end_time and curr_end_time > workday_end_dt:
        if not service_block_after_first_afterhours_nontransport:
            service_block_after_first_afterhours_nontransport = True
        else:
            return True, True, labors_df, moves_df, curr_end_time, curr_end_pos

    duration = get_nontransport_labor_duration(
    ...
    return True, False, new_labors, new_moves, new_end_time, labor_pos
