    ...
    **kwargs
    ...
    for _, row in df_sorted.iterrows():
        ...
        if row['labor_category'] == 'VEHICLE_TRANSPORTATION':
            pick, dist_dict = get_driver_wrapper(drivers, row, prev_end, TIEMPO_PREVIO, TIEMPO_GRACIA,
                                      ALFRED_SPEED, dist_method, dist_dict,
                                      alpha, assignment_type, **kwargs)
            ...
            astart, aend, dist_km = assign_task_to_driver(
                                                    ...
                                                    VEHICLE_TRANSPORT_SPEED, dist_method, 
                                                    dist_dict, **kwargs
                                                    )
    ...
    df_moves = build_driver_movements(df_result, directorio_df, day_str, dist_method, 
...
def build_driver_movements(
    ...
    **kwargs :
    ...
    if driver_init_mode == 'historic_directory':
        ...
        df_city = directory_df[directory_df['city'] == city]
    ...
    for _, row in labors_df.dropna(subset=[start_col]).sort_values(start_col).iterrows():
        ...
        dkm, _ = distance(prev_pos, row['map_start_point'],
                          method=DISTANCE_METHOD, dist_dict=dist_dict, **kwargs)
        dkm = 0.0 if (pd.isna(dkm) or math.isnan(dkm)) else dkm
        travel_time = timedelta(minutes=(dkm / ALFRED_SPEED * 60)) if dkm > 0 else timedelta(0)
...
def remove_drivers(df: pd.DataFrame, driver_names):
    """
